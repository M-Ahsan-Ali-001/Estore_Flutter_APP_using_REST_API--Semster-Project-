...
class sh_reset extends StatefulWidget {
  final String name;
  final String email;
  const sh_reset({Key? key,required this.name,required this.email}) : super(key: key);

  @override
  State<sh_reset> createState() => _sh_resetState();
}

class _sh_resetState extends State<sh_reset> {
  ...
  Future chng_p()async {
    ...
    if(_p1.text.toString()==_p2.text.toString()){
      var url= Uri.parse("http://10.4.30.54/flutter_mysql_server/sp_rst.php");
